...
class CollegePost extends State<CollegePostState> {
  ...
  getPosts(String location, [int start]) async {
    ...
    if (start != null) {
      try {
        response =
            await apiService.getCollegePostTime(location, context, start);
      } catch (e) {
        _scaffoldKey.currentState.showSnackBar(fail);
      }
    } else {
      response = await apiService.getCollegePostTime(location, context);
    }
    return response;
  }
                    ...
                    children: [
                      ...
                      Row(children: [
                        ...
                        RaisedButton(
                          onPressed: () {
                            Navigator.pushReplacementNamed(
                                context, CollegeRatingPost.route,
