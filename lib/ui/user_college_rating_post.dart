...
class UserCollegeRatingPostState extends StatefulWidget {
  UserCollegeRatingPostState(this.location);
  final String location;
  UserCollegeRatingPost createState() => UserCollegeRatingPost();
}

class UserCollegeRatingPost extends State<UserCollegeRatingPostState> {
  static const route = '/user/college/rating';
  ...
  getPosts(String locUser, [int start, String location, int rating]) async {
    ...
    if (start != null) {
      try {
        response = await apiService.getSchoolUserRating(locUser, start, location, rating);
      }
      catch(e) {
      ...
      response = await apiService.getSchoolUserRating(locUser);
    }
    return response;
  }
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                if (index == items.length && items.length >= 5) {
                                  ...
                                  var newPosts = getPosts(items[startIndex].schoolUser,
