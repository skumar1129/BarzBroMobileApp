...
class BarCityRatingPost extends State<BarCityRatingPostState> {
  ...
  getPosts(String locBar, [int start, String location, int rating]) async {
    ...
    if (start != null) {
      try {
        response = await apiService.getCityBarRating(
            locBar, context, start, location, rating);
      } catch (e) {
        _scaffoldKey.currentState.showSnackBar(fail);
      }
    } else {
      response = await apiService.getCityBarRating(locBar, context);
    }
    return response;
  }
