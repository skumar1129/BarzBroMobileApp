...
import './user_city_post.dart';

class UserCityRatingPostState extends StatefulWidget {
  UserCityRatingPostState(this.location);
  final String location;
  UserCityRatingPost createState() => UserCityRatingPost();
}

class UserCityRatingPost extends State<UserCityRatingPostState> {
  static const route = '/user/city/rating';
  ...
  getPosts(String locUser, [int start, String location, int rating]) async {
    var response;
    if (start != null) {
      try {
        response = await apiService.getCityUserRating(
            locUser, context, start, location, rating);
      } catch (e) {
        _scaffoldKey.currentState.showSnackBar(fail);
      }
    } else {
      response = await apiService.getCityUserRating(locUser, context);
    }
    return response;
  }
