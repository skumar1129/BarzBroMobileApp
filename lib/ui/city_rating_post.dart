...
import './city_post.dart';

class CityRatingPostState extends StatefulWidget {
  CityRatingPostState(this.location);
  final String location;
  @override 
  CityRatingPost createState() => CityRatingPost();
}

class CityRatingPost extends State<CityRatingPostState> {
  static const route = '/city/rating';
  ...
  getPosts(String location, [int start, int rating]) async {
    var response;
    
    if (start != null) {
      try {
        response = await apiService.getCityPostsRating(location, start, rating);
      }
      catch(e) {
      ...
      response = await apiService.getCityPostsRating(location);
    }
    return response;
  }
              ...
              if (snapshot.hasData) {
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          RaisedButton(
                            onPressed: () {
                              Navigator.pushReplacementNamed(context, CityPost.route,
                      ...
                      Expanded(
                        child: Image(image: AssetImage('assets/img/city_page.jpg'),),
                      )
                    ],
                  )
                  );
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                if (index == items.length && items.length >= 5) {
                                  ...
                                  var newPosts = getPosts(items[startIndex].location, 
                ...
                return Expanded(
                  child: Column(
                    ...
                    Expanded(child: Image(image: AssetImage('assets/img/city_page.jpg'),),)
                    ],
                  )
                );
              }
