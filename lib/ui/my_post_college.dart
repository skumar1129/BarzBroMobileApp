...
class MyPostCollege extends State<MyPostCollegeState> {
  ...
  getPosts([int start, String location]) async {
    ...
    if (start != null && location != null) {
      try {
        response = await apiService.getSchoolUser(context, start, location);
      } catch (e) {
        _scaffoldKey.currentState.showSnackBar(fail);
      }
    } else {
      response = await apiService.getSchoolUser(context);
    }
    return response;
  }
  ...
  editPost(post) {
    Navigator.pushReplacementNamed(context, EditCollegePost.route,
        arguments: post);
  }
                ...
                if (items.length == 0) {
                  return Expanded(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        'Here is a place to see all the posts you make in Colleges!',
                        style: TextStyle(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            if (index == items.length && items.length >= 4) {
                              ...
                              String goodRegion =
                                  utf8.decode(items[index].region.codeUnits);
                              String goodBar =
                                  utf8.decode(items[index].bar.codeUnits);
                              String goodSchool =
                                  utf8.decode(items[index].school.codeUnits);
                              return Card(
                                color: Colors.black,
                                child: Column(
                                  children: <Widget>[
                                    ...
                                    Row(
                                      children: <Widget>[
                                        ...
                                        Flexible(
                                          child: Text(
                                            '${capitalize(goodRegion)}, $goodSchool',
                                            style: TextStyle(
                                                color: Colors.white,
                                                fontFamily:
                                                    'Merriweather-Regular'),
