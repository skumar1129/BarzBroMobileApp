...
class BarCollegeRatingPostState extends StatefulWidget {
  BarCollegeRatingPostState(this.location);
  final String location;
  @override
  BarCollegeRatingPost createState() => BarCollegeRatingPost();
}

class BarCollegeRatingPost extends State<BarCollegeRatingPostState> {
  static const route = '/bar/college/rating';
  ...
  getPosts(String schoolBar, [int start, String location, int rating]) async {
    var response;
    if (start != null) {
      try {
        response = await apiService.getCollegeBarRating(
            schoolBar, context, start, location, rating);
      } catch (e) {
        _scaffoldKey.currentState.showSnackBar(fail);
      }
    } else {
      response = await apiService.getCollegeBarRating(schoolBar, context);
    }
    return response;
  }
                ...
                if (items.length == 0) {
                  return Expanded(
                    ...
                    children: [
                      ...
                      Text(
                        'No posts for ${capitalize(splitLocation[1])} in ${splitLocation[0]} yet (Make sure your spelling is correct)',
                        style: TextStyle(
