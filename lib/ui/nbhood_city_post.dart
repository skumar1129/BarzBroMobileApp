...
import './nbhood_city_rating_post.dart';
import 'package:strings/strings.dart';

class NbhoodCityPostState extends StatefulWidget {
  NbhoodCityPostState(this.location);
  final String location;
  @override
  NbhoodCityPost createState() => NbhoodCityPost();
}

class NbhoodCityPost extends State<NbhoodCityPostState> {
  static const route = '/nbhood/city';
  ...
  getPosts(String locNeighborhood, [int start, String location]) async {
    var response;
    if (start != null) {
      try {
        response = await apiService.getCityNbhoodTime(locNeighborhood, start, location);
      }
      catch(e) {
      ...
      response = await apiService.getCityNbhoodTime(locNeighborhood);
    }
    return response;
  }
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          RaisedButton(
                            onPressed: () {
                              Navigator.pushReplacementNamed(context, NbhoodCityRatingPost.route,
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                if (index == items.length && items.length >= 5) {
                                  ...
                                  var newPosts = getPosts(items[startIndex].locNeighborhood,
