...
import './college_post.dart';
import '../widgets/post_card.dart';

class CollegeRatingPostState extends StatefulWidget {
  CollegeRatingPostState(this.location);
  final String location;
  CollegeRatingPost createState() => CollegeRatingPost();
}

class CollegeRatingPost extends State<CollegeRatingPostState> {
  static const route = '/college/rating';
  ...
  getPosts(String location, [int start, int rating]) async {
    ...
    if (start != null) {
      try {
        response = await apiService.getCollegePostRating(
            location, context, start, rating);
      } catch (e) {
        _scaffoldKey.currentState.showSnackBar(fail);
      }
    } else {
      response = await apiService.getCollegePostRating(location, context);
    }
    return response;
  }
                ...
                if (items.length == 0) {
                  return Expanded(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Image(
                          image: AssetImage('assets/img/college_page.jpg'),
                        ),
                      )
                    ],
                  ));
